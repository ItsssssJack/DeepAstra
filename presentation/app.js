...
const [benchmarks,prompts]=await Promise.all(['benchmarks.json','prompts.json'].map(x=>fetch(x+'?v=16').then(r=>{if(!r.ok)throw Error(x);return r.json()})));
...
function syncFilms(){ $$('[data-film]').forEach(v=>{if(!paused&&!document.hidden&&v.closest('.is-visible')&&!v.closest('[hidden]'))v.play().catch(()=>{});else v.pause()}); }
function motion(){lottieMotions.forEach(({el,anim})=>{if(paused)anim.pause();else if(el.closest('.is-visible'))anim.play()});document.body.classList.toggle('motion-paused',paused);syncFilms()}motion();
...
function installBenchmarkCarousel(prefix,barsId,rows){
 ...
 function render(){
  ...
  bars.innerHTML=fixedOrder.map(i=>{
   ...
   return `<div class="ranked-model ${i===0?'seek':''} ${identity}" data-model="${names[i]}"><div class="ranked-column"><div class="ranked-fill" style="height:${Math.max(0,height)}%;${top?'top':'bottom'}:${anchor}%">${i===0?silkFilm():''}<strong class="ranked-value ${top?'below':''}">${v}${b.suffix}</strong></div></div><span class="ranked-name"><span class="model-crest"><img class="brand" src="assets/${logo}.svg" alt=""></span><span>${names[i]}</span></span></div>`;
  }).join('');
